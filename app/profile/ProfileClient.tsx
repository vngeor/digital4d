...
import { User, Mail, Phone, MapPin, Calendar, Edit2, ArrowLeft, Globe, Building, Cake, Lock, ChevronRight } from "lucide-react"
...
interface ProfileClientProps {
  ...
  translations: {
    ...
    secretDealsView: string
    secretDealsCount: string
  }
  activeDealsCount: number
}

export function ProfileClient({ user, translations: t, activeDealsCount }: ProfileClientProps) {
      ...
      <div className="relative pt-16 sm:pt-20 md:pt-24 pb-16">
        ...
        {/* Secret Deals teaser */}
          <Link
            ...
            className="mt-6 md:mt-8 block glass rounded-2xl border border-amber-500/20 bg-amber-500/5 hover:bg-amber-500/10 hover:border-amber-500/30 p-5 transition-all group"
          >
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-semibold text-white">{t.secretDealsTitle}</h3>
                  <p className="text-sm text-slate-400">{t.secretDealsSubtitle}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                {activeDealsCount > 0 && (
                  <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-amber-500 text-white text-xs font-black">
                    {activeDealsCount}
                  </span>
                )}
