...
export function RecentlyViewedSection({ categories = [] }: { categories?: QVCategory[] } = {}) {
    ...
    const [couponMap, setCouponMap] = useState<Record<string, { type: string; value: string; currency: string | null } | null>>({})
    ...
    useEffect(() => {
        const viewed = getRecentlyViewed().slice(0, 8)
        setProducts(viewed)
        // Fetch coupon badges for displayed products
        if (viewed.length > 0) {
            const ids = viewed.map(p => p.id).join(",")
            fetch(`/api/products/badges?ids=${ids}`)
                .then(r => r.ok ? r.json() : {})
                .then(data => setCouponMap(data))
                .catch(() => {})
        }
    }, [])
