...
- **Secret Deals** (`app/profile/secret-deals/`): dedicated voucher page at `/profile/secret-deals`. Server component fetches coupon notifications with full edge-case handling (dedup by couponId, startsAt gate, perUserLimit via `CouponUsage.email`, expired/exhausted filter). Also bulk-fetches brand and category names for restriction chips. Client component (`SecretDealsClient`) renders compact 3-column card grid: source emoji badge, big amber discount value, monospace code + copy button, restriction chips (🟢 All products / 🔵 category names / 🟣 brand names / ⚪ X selected products / min purchase), countdown timer (SSR-safe: initializes to `null`, populates in `useEffect` to avoid hydration mismatch), expiry date, "Shop Now" CTA. Used coupons grayed-out; expired excluded. Header dropdown (desktop) and full-screen mobile menu both link to the page via `t("mySecretDeals")` with `Tag` icon (amber styling in dropdown).
